...
func TestCreateKeychain(t *testing.T) {
	directory, err := ioutil.TempDir("", "appsigner-test")
	if err != nil {
...
func TestInstallCertificate(t *testing.T) {
	...
	certsha1, certpath, err := extractFixtureCertificate(directory)
	if err != nil {
		t.Skip()
		return
	}
...
func extractFixtureCertificate(tempdir string) (string, string, error) {
	...
	err = ioutil.WriteFile(certPath, profileAndCertificate.P12Bytes, 0644)
	if err != nil {
		return "", "", err
	}
	return certsha1, certPath, nil
}
