...
func TestFindAppfolder(t *testing.T) {
	...
	assert.False(t, codesign.ContainsAppstoreApp(appPath))
	assert.False(t, codesign.ContainsAppstoreApp(notAnAppPath))
	assert.True(t, codesign.ContainsAppstoreApp(appStoreAppPath))

}

func setUpExampleAppDir() (string, string, string, string, error) {
	tempdir, err := ioutil.TempDir("", "appsigner-findappfolder-test")
	if err != nil {
		return "", "", "", "", err
	}
