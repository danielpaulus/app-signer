...
//is only temporary.
const KeychainPassword = "random-pwd"
...
//security list-keychain -s [keychain1] [keychain2]...  command
...
func AddX509CertificateToKeychain(keychain string, certificate string, profilePassword string) error {
	_, err := executeSecurity("import", certificate, "-k", keychain, "-P", profilePassword, "-T", codesignPath)
	return err
}
