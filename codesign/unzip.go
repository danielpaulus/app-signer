...
//ExtractZip takes a io.ReaderAt and a length to extract a zip archive to a
...
func ExtractZip(zipFile io.ReaderAt, length int64) (time.Duration, string, error) {
	destination, err := ioutil.TempDir("", "appsign-ipa-extract")
	if err != nil {
		return 0, "", err
	}
